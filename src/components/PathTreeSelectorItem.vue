...
<template>
  <li>
    <button @click="onClick" class="btn btn-ghost text-md cursor-pointe w-full justify-start rounded-selector" :class="colorClass">
    ...
    <ul v-if="pathTree.type === 'dir' && showChildren">
      <PathTreeItem v-for="child in pathTree.children" :key="child.path" :path-tree="child" :depth="depth + 1" :selected-path="selectedPath"
